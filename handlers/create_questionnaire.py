        ...
        if DataBaseWork().get_data_from_profiles_table('changes', message.from_user.id) == 3:
            ...
            if await is_russian(message.text) == True:
                ...
                await bot.send_photo(message.from_user.id,
                                     photo='AgACAgIAAxkBAAMEY9Z58jjwxsC-qLuaoeqFhW6zKOcAAuzEMRtpIrBKqMjIapZ4loYBAAMCAANzAAMtBA',
                                     caption='Выпиши словами из списка (на фото) 3 пункта, '
                                             ...
                                             'Учу корейский\n'
                                             'Корн-дог\n\n'
                                             'Enhypen - группа, благодаря которой я влюбилась в k-pop. '
                                             'Очень люблю их творчество. Учу язык чтобы уехать в Корею на обучение. '
                                             'За корн-дог могла бы и душу дьяволу продать... 💞')
            else:
                await message.answer('Напиши твой город. Без указания страны, региона, цифр и нарусском языке👇',
                                     reply_markup=buttons.remove_markup)
        ###
